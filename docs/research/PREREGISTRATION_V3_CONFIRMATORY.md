# Preregistration V3 — Confirmatory ShadowNAV

**Status: FINAL — APPROVED FOR PHASE B FREEZE; PHASE C/D REVEAL NOT YET AUTHORIZED.**

This document incorporates the approved pre-Phase-B dependence and duplicate-vote corrections requested after the outcome-blind human event review. It is finalized with the Phase B sample freeze in the same commit. It does not authorize access to numeric target-current outcomes or a target-outcome reveal. No result may be calculated from target same-quarter marks until the separate Phase C structural review and freeze are completed and Phase D is explicitly authorized.

The final Phase A consensus SHA-256 is `2a0c763e423b5616b3f9093f54a0073d5e8577b0fe4f5769fb2ca60ff26f9591`. It contains 37 included observations in 34 source-event clusters and 20 unique normalized borrowers; three uncertain observations/clusters remain excluded and retained in the audit trail. These counts and the Phase B freeze are outcome-blind. Target-current structure and numeric marks had not been opened when this document and sample were frozen.

## Approval and frozen implementation record

- Approval status: **APPROVE FOR PHASE B FREEZE**.
- Independent approval: Claude confirmed on 2026-08-15 that all three blocking methodological requirements were satisfied.
- Approved Draft V2 SHA-256: `ccee7b3ffbabc5b5f785a7ab0951cabb46cc82366a03c650f154155ff070b7e3`.
- Final Phase A consensus SHA-256: `2a0c763e423b5616b3f9093f54a0073d5e8577b0fe4f5769fb2ca60ff26f9591`.
- Frozen included-sample SHA-256: `011da2ab9ccc39f5c2530295fee1b555377f4a2b36a302e45183873af603a670`.
- Borrower-cluster distribution SHA-256: `cfd3b83d03c47041427dce98f731e1c1d9423408003aac946a10339d26a0344d`.
- Passing duplicate-vote audit SHA-256: `d62f9e30352ffb8b9574858b30881032022067c0a69c5c8d474695722d0a96e1`.
- Sample-generation script SHA-256: `1afae3c8905b2189bbbbc6bf498545e42256b14ec87718e3c03b1ee6b6f52590`.
- Frozen evaluator SHA-256: `bcea297f43603316d4d3bc5fef9762bc2749eaddf36a3253222af66e8f132615`.
...
Phase A is complete. Its signed final consensus has 37 included observations, 34 included source-event clusters, and three uncertain observations/clusters retained outside the proposed sample. Human labels and inclusion decisions are unchanged by the pre-Phase-B correction and are frozen by the Phase B record associated with this final document.
...
The borrower counts must be derived from final consensus, not hardcoded. The freeze record must contain no target-current structural or numeric data. A passing duplicate-vote audit is necessary but does not itself authorize or create Phase B. The Phase B freeze is created in the same commit as this final preregistration, before Phase C begins.
...
The evaluator is prepared and tested only on synthetic data. Its SHA-256 is frozen above and in the separate Phase B record. The outcome-blind included sample, final Phase A consensus, this preregistration, and their generating code are bound together by `data/day4/confirmatory_sample_freeze.json` in the same Phase B commit.

The only next authorized action after that commit is preparation of the strictly non-numeric Phase C structural-review packet for exactly the frozen IDs. Phase D numeric reveal, results calculation, and any results tag remain prohibited. Target-current structure and numeric marks were not opened before the Phase B freeze.
