...
## 8. Later reveal phases (not authorized by this draft)

1. Final human event consensus is frozen first.
2. Final STRICT and SUPPORTING included samples, IDs, layer membership, consensus hash, this preregistration, evaluator, and generation code are frozen in a separate sample-freeze commit.
3. Target-current non-valuation structure is revealed for exactly the frozen IDs, independently reviewed, adjudicated without marks, and frozen in a separate structural-consensus commit.
4. Only a complete authorization record may permit the frozen evaluator to open numeric outcomes.

The evaluator must verify its own SHA-256, all bound file hashes and freeze commits, exact frozen-ID equality, and equality between numeric-file structural labels and separately frozen structural consensus. Numeric outcomes cannot redefine structure.

## 9. Candidate-set boundary recorded now

This preparation commit records SHA-256 values for:

- the blind review packet;
- STRICT candidate observation-ID set;
- SUPPORTING candidate observation-ID set;
- ordered opaque review-ID list;
- source-event-cluster sets;
- borrower sets;
- this draft; and
- the prepared evaluator.

These hashes are candidate-universe provenance, not a final sample freeze. Human review has not occurred and target-current structure and numerical outcomes remain unopened.

## 10. Prohibitions at this stage

- No final Day 5 sample freeze.
- No target-current structural reveal.
...
- No result tag.
- No membership changes after outcome reveal.
